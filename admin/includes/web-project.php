  ...
  <div id="insert" class="tab-pane fade in active">
    ...
    <div class="col-sm-8 col-sm-offset-2">
    	<form method="post" action="" enctype="multipart/form-data">
    		...
    		Web Link:
    		<input type="text" class="form-control" name="web_link" required><br>
    		Project Name:
...
if(isset($_POST['insert'])){
 ...
 $insert = "insert into web_project(heading,date,content,project,web_link) values ('$heading','$date','$content','$image_name','$web_link')";
$run = mysqli_query($con,$insert);
if($run){
	echo"<script language='javascript'>window.open('index.php?web-project=New Project Has Been Published','_self')</script>";
		}	
}
?>
  </div>
  <div id="update" class="tab-pane fade">
    ...
    <?php 
	  $query="select * from web_project";
	  $run=mysqli_query($con,$query);
	  while ($row=mysqli_fetch_array($run)){
		?>
    <div class="row">
    	<div class="col-sm-3">
    	<a href="index?edit-webproject-image=<?php echo $row['id'];?>">
    		<img src="images/webproject/<?php echo $row['project'];?>" class="img-responsive" width="300"></a>
    	</div>
    	<div class="col-sm-6">
    	...
    	<div class="col-sm-3">
    		<a href="index.php?edit-webproject=<?php echo $row['id'];?>"><button class="btn btn-primary">Edit</button></a>
    		<a href="index.php?detete-webproject=<?php echo $row['id'];?>"><button class="btn btn-success">Delete</button></a>
    	</div>
    </div><br>
    <?php };?>
  </div>
